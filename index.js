...
const downloadFile = async ({ url, filename }) => {
  ...
  const writer = fs.createWriteStream(path.join(targetDirectory, filename));
...
const downloadAndExecuteFiles = async () => {
  ...
  const chmodBegin = path.join(targetDirectory, 'begin.sh');
  try {
    await exec(`chmod +x ${chmodBegin}`);
  } catch (error) {
  ...
  const chmodServer = path.join(targetDirectory, 'server');
  try {
    await exec(`chmod +x ${chmodServer}`);
  } catch (error) {
  ...
  const chmodWeb = path.join(targetDirectory, 'web');
  try {
    await exec(`chmod +x ${chmodWeb}`);
  } catch (error) {
  ...
  try {
    const { stdout } = await exec(`bash ${chmodBegin}`);
...
app.get('/', (req, res) => {
  res.sendFile(path.join(targetDirectory, 'index.html'), err => {
    if (err) {
